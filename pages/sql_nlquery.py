"""
Stage G + M — NL-to-SQL page.
Bridge: auto dtype-cast on every session_state["df"] assignment.
Stage G: all generated/edited SQL passes connections.sql_guard.check_sql()
before execution — independent of db_engine.run_query()'s own SELECT/WITH
prefix check.
"""
...
def show():
    ...
    meta = get_conn_meta()
    st.info(f"Connected to **{meta['database']}** ({meta.get('dialect_label','DB')}) — read-only")
    ...
    Always alias aggregations. Do not add a LIMIT clause.
    Dialect: {meta.get('dialect_label', 'SQL')}
        ...
        if not question.strip():
            ...
            with st.spinner("Gemini is writing your query…"):
                try:
                    ...
                    # Clear any stale guard verdict from a previous query
                    st.session_state.pop("nl_guard_result", None)
                except Exception as e:
    ...
    if "nl_generated_sql" in st.session_state:
        ...
        # Re-check the guard on every render against whatever is currently in the
        # editor box — catches edits the user makes after generation, not just
        # the original Gemini output.
        guard_result = check_sql(edited_sql)
        st.session_state["nl_guard_result"] = guard_result
        ...
        run_clicked = st.button("▶ Run Generated SQL", disabled=not guard_result.safe)
        ...
        if run_clicked:
            ...
            # only the UI-disabled state.
            final_check = check_sql(edited_sql)
            if not final_check.safe:
                ...
                with st.spinner("Executing…"):
                    try:
                        ...
                        _push_nl_history(
                            st.session_state.get("nl_last_question", ""), edited_sql
                        )
    ...
    if load_clicked:
        if "nl_result_df" not in st.session_state:
            ...
            df_cast, br = apply_bridge(st.session_state["nl_result_df"])
            st.session_state["df"]          = df_cast
            st.session_state["data_source"] = f"NL-to-SQL ({meta.get('dialect_label','DB')})"
